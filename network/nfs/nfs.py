...
def get_inverse_window(forward_window, frame_length, frame_step):
    # 用于计算合成窗口（inverse window）的函数，
    # 合成窗口通常用于信号处理中的反变换过程，以修正因分析窗口引起的估计误差
...
class NFS(nn.Module):
    ...
    def forward(self, pos, dry):
        ...
        """ Fourier Features """
        if not self.wo_lff:
            pos = rearrange(pos, 'b (p c) t -> (b t) p c', c=1)  # (batch*96 , 7 , 1）
...
class FourierShift(nn.Module):
    ...
    def forward(self, x, c, g, b):
        ...
        x = x.tile(1, self.channel, 1)  # torch.Size([9*batchsize, 128, 9600])
